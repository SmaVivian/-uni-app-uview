			...
			<view class="item" direction="column" v-for="(item, i) in dataList" :key="i" title="列表文字" @click="goDetail(item)">
				<view class="item-head">
					<view class="item-head-l">
						<u-lazy-load :image="item.icon||errorImg" :error-img="errorImg" border-radius="50" height="64" style="width:64rpx"></u-lazy-load>
						<text class="item-head-name">{{item.origin}}</text>
					</view>
					<text class="item-head-r">
						{{item.published_at | DateFormat('yyyy-MM-dd')}}
						<!-- {{$u.timeFormat(item.published_at, 'yyyy-mm-dd')}} -->
					</text>
				</view>
				
				<view class="item-body">
					<view class="title u-line-1">{{item.title}}</view>
					<view class="desc u-line-3">{{item.flag || ''}}</view>
		...
		methods: {
			...
			getDataList(operate) {
				...
				if (this.last_id) {
					//说明已有数据，目前处于上拉加载
					// this.loadingType = 'loading';
					aaa.minId = this.last_id;
					aaa.time = new Date().getTime() + '';
					aaa.pageSize = 10;
				}
				
				this.loadingType = 'loading'
				// this.$u.get('/vi/article/load', {
				this.$u.get('/news', {
					// provinceId: '', // 天津市
					...aaa,
					tag: this.tabVal,
